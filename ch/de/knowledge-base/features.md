...
Die **verfügbaren Features** auf der linken Seite können Sie selbst aktivieren <iconify-icon inline icon='mdi-toggle-switch'/> oder - sofern kostenpflichtig - im Tab <a href="/ch/de/get-started/set-up-your-account#abonnement-verwalten" target="_blank">Abonnement</a> jederzeit selbstständig hinzubuchen. Ergänzungen des laufenden Vertrages werden immer anteilig zur aktuellen Laufzeit berechnet. 

Die **Vorschau-Features** auf der rechten Seite stehen in einer Alpha-Version für die Evaluierung durch unsere Benutzer zur Verfügung. Wenn Sie Interesse haben, uns bei der Entwicklung mittels Anwenderfeedback zu unterstützen, melden Sie sich bitte gern unter über unsere Servicehotline [+41 44 500 4446](tel:+41445004446) oder per Mail an <info@madaster.ch> und wir schalten Ihnen den Zugriff frei. Im Folgenden sind diese Features mit einem Stern markiert.
...
Dieses Feature ermöglicht die Interaktion mit der Madaster-Plattform über deren integrierte Schnittstelle mittels Code oder aus anderen Anwendungen unter Verwendung der bereitgestellten <a href="/ch/de/api/index" target="_blank">API-Endpunkte</a>.

Ein <a href="/ch/de/api/index#token" target="_blank">API-Token</a> muss generiert werden, damit sich die Anwendung Ihrer Wahl als Benutzer mit bestimmten Rechten und Rollen identifizieren kann. Dann gibt es alle Arten von Aktionen, die über diese API durchgeführt werden können. Folgend ein paar Beispiele.
...
> Hinweis: Bitte achten Sie darauf, dass außerdem der <a href="/ch/de/knowledge-base/features#esg" target="_blank">ESG-Tab</a> aktiviert ist, um Zugriff auf das dort integrierte CRREM-Tool zu haben.
...
Auf **Objektebene** kann im Tab **ESG** im Dropdown diese Berichtsart ausgewählt werden.Derzeit kann Madaster Sie im Zertifizierungsprozess in den folgenden Teilbereichen unterstützen:
...
1. Teilbereiche der KPIs 1-9 des Kriterienkatalogs können ausgewertet werden und werden in einem Dashboard auf der Plattform dargestellt.
1. Die vorhandenen Daten können über die Schaltfläche **Gebäuderessourcenpass generieren** <iconify-icon inline icon='mdi-cog-outline'/> exportiert werden. In diesem Anwendungsfall werden die Tabellenfelder, für welche Informationen durch die Nutzung der Madaster-Plattform vorliegen, automatisch befüllt. 
1. Nach Fertigstellung steht Ihnen der befüllte Gebäuderessourcenpass gemäß der Excelvorlage der DGNB im **Dossier** zum Download zur Verfügung.
1. Fehlende Informationen können in der generierten Excel nach dem **Download** außerhalb der Plattform ergänzt werden.

> Weitere Kriteriengruppen des Kriterienkatalogs, wie bspw. Teilbereiche des ENV 1.1, ECO 2.7 und des TEC 1.6, befinden sich aktuell in der Entwicklung.
...
* <a href="/ch/de/knowledge-base/features#crrem-carbon-rist-real-estate-monitor" target="_blank">CRREM</a>
* <a href="/ch/de/knowledge-base/features#levels" target="_blank">Level(s)</a>
* <a href="/ch/de/knowledge-base/features#dgnb" target="_blank">DGNB</a>

## Infrastruktur*

Dieses Feature ermöglicht neben Gebäuden auch andere <a href="/ch/de/knowledge-base/classifications/nutzungsarten" target="_blank">Objekttypen</a> auf der Madaster-Plattform registrieren und dokumentieren zu können. Durch das Hochladen einer Excel- oder IFC-Quelldatei, können **Infrastrukturobjekte** nach Verknüpfung mit passenden Produktdaten regulär bezüglich Masse, Umweltwirkung, Zirkularität und finanziellem Restwert ausgewertet werden.

*Es handelt sich um ein **Vorschau-Feature**. Bei Interesse kontaktieren Sie bitte den Support.

> Hinweis: Aktuell suchen wir **erfahrene Partner**, die uns bei der Entwicklung begleiten und für Interviews zu Themen wie spezifische Dateiformate, Klassifikationen, Nachhaltigkeitsschwerpunkte und Reportings in den diversen **Kategorien** zur Verfügung stehen.
...
> Hinweis: Bitte achten Sie darauf, dass außerdem der <a href="/ch/de/knowledge-base/features#esg" target="_blank">ESG-Tab</a> aktiviert ist, um Zugriff auf die dort integrierten Level(s)-Berichte zu haben.
