...
module LastModified
  class Determinator
    ...
    def absolute_path_to_article
      sanitized_path = Jekyll.sanitized_path(@site_source, @page_path)
      realpath = File.realpath(sanitized_path)
      if realpath != sanitized_path
        sanitized_path = realpath
      end

      @absolute_path_to_article ||= sanitized_path
    end

    def relative_path_to_article
      @relative_path_to_article = Pathname.new(absolute_path_to_article)
        .relative_path_from(
