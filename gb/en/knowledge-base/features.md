...
At **<iconify-icon inline icon='mdi-briefcase-variant'/> Account level** you will find the **Features** tab including an overview of the 'available features' and 'preview features'. 

The difference being that **preview features** are still in development and may therefore not be retained in their current form, or may even be discontinued at some point. We would appreciate your feedback on these preview features if anything does not work as you think it should, or if anything is missing or incorrect. These features are marked with an asterisk below.

To be able to use a feature it needs to be **toggled on <iconify-icon inline icon='mdi-toggle-switch'/>**. Features are occasionally accessed as a 'paid feature' or by 'invitation only', so please contact your local Madaster service if you are interested.
...
*This is a **preview feature**. If you are interested, please contact your local Madaster service.

## 3D Insights

Not to be confused with our **3D** Tab, which visualizes the IFC-File uploaded in the building folder, the **3D Insights** allows a 3D visual analysis of which products in the IFC-file have the largest influence on your building's environmental impact and circularity score.
...
This **paid feature** can be added via the Subscription tab.
...
For more information about API for Madaster, check out the specific [API help-pages](../api/index.md)
...
On building level, in the **ESG** Tab, a method of reporting (in this case: **CRREM**) can be chosen. In the button **Show settings** the user needs to **Input** the data for the building characteristics, energy procurement, fugitive emissions and renewable energies. This data is combined with a global warming goal and respectively a default 
...
To document infrastructure projects on Madaster, users can switch on the Infra feature, which in addition to buildings enables other object types (e.g.: Engineering Structure, Terrain, Transport, or Cable/Pipe) to be set up. By uploading an Excel or IFC-file containing the build object and linking it to products in a choosen database, an evaluation concerning the Mass, Circularity and Environment can be made.
